...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--config URL] [--repo owner/repo] [--branch branch-name] [--lang locale]"
            exit 1
            ;;
    esac
done
...
set_language() {
    local raw_lang
    raw_lang="$(resolve_requested_language)"
    raw_lang="${raw_lang%%.*}"
    raw_lang="${raw_lang%%_*}"
    raw_lang="$(to_lower "$raw_lang")"

    if is_supported_language "$raw_lang"; then
        CURRENT_LANG="$raw_lang"
    else
        CURRENT_LANG="en"
        echo -e "${YELLOW}⚠${NC} $(translate "installer.lang.unsupported" "$raw_lang")"
    fi

    echo -e "${BLUE}ℹ${NC} $(translate "installer.lang.active" "$CURRENT_LANG")"
}
...
translate_lookup() {
    ...
    local key="$2"
    case "${lang}:${key}" in
        "en:connection.chosen.ble") echo "Selected: Bluetooth Low Energy (BLE)" ;;
        "en:connection.chosen.serial") echo "Selected: Serial Connection" ;;
        "en:connection.chosen.tcp") echo "Selected: TCP Connection" ;;
        "en:connection.header") echo "Device Connection Configuration" ;;
        "en:connection.menu") echo "How would you like to connect to your MeshCore device?\n\n  1) Bluetooth Low Energy (BLE) - Recommended for T1000 devices\n     • Wireless connection\n     • Works with MeshCore T1000e and compatible devices\n\n  2) Serial Connection - For devices with USB/serial interface\n     • Direct USB or serial cable connection\n     • More reliable for continuous operation\n\n  3) TCP Connection - For network-connected devices\n     • Connect to your node over the network\n     • Works with ser2net or other TCP-to-serial bridges" ;;
        "en:connection.prompt.choice") echo "Select connection type [1-3]" ;;
        "en:connection.prompt.scan_ble") echo "Would you like to scan for nearby BLE devices?" ;;
        "en:installer.cancelled") echo "Installation cancelled." ;;
        "en:installer.header.install_method") echo "Installation Method" ;;
        "en:installer.header.installing_files") echo "Installing Files" ;;
        "en:installer.info.downloading") echo "Downloading from GitHub (%s @ %s)..." ;;
        "en:installer.info.install_dir_selected") echo "Installation directory: %s" ;;
        "en:installer.intro") echo "This installer will help you set up MeshCore Packet Capture." ;;
        "en:installer.lang.active") echo "Language set to %s." ;;
        "en:installer.lang.unsupported") echo "Language %s not supported. Falling back to English." ;;
        "en:installer.menu.install_method") echo "Choose your preferred installation method:\n\n  1) System Service (recommended for production)\n     • Runs automatically on boot\n     • Managed by systemd (Linux) or launchd (macOS)\n     • Automatic restart on failure\n\n  2) Docker Container (recommended for development/testing)\n     • Isolated environment\n     • Easy to update and manage\n     • Works on Linux, macOS, and Windows\n\n  3) Manual installation only\n     • No automatic startup\n     • Run manually when needed" ;;
        "en:installer.prompt.dir_exists") echo "Directory already exists. Reinstall/update?" ;;
        "en:installer.prompt.install_dir") echo "Installation directory" ;;
        "en:installer.prompt.install_method") echo "Choose installation method [1-3]" ;;
        "en:installer.summary.config") echo "Configuration file: %s/.env.local" ;;
        "en:installer.summary.header") echo "Installation Complete!" ;;
        "en:installer.summary.install_dir") echo "Installation directory: %s" ;;
        "en:installer.title") echo "MeshCore Packet Capture Installer v%s" ;;
        "es:connection.chosen.ble") echo "Seleccionado: Bluetooth Low Energy (BLE)" ;;
        "es:connection.chosen.serial") echo "Seleccionado: Conexión serie" ;;
        "es:connection.chosen.tcp") echo "Seleccionado: Conexión TCP" ;;
        "es:connection.header") echo "Configuración de conexión del dispositivo" ;;
        "es:connection.menu") echo "¿Cómo te gustaría conectar tu dispositivo MeshCore?\n\n  1) Bluetooth Low Energy (BLE) - Recomendado para equipos T1000\n     • Conexión inalámbrica\n     • Compatible con MeshCore T1000e y dispositivos similares\n\n  2) Conexión serie - Para dispositivos con interfaz USB/serie\n     • Cable USB o serie directo\n     • Mayor confiabilidad para operación continua\n\n  3) Conexión TCP - Para dispositivos conectados a la red\n     • Conéctate a tu nodo mediante la red\n     • Funciona con ser2net u otros bridges TCP a serie" ;;
        "es:connection.prompt.choice") echo "Selecciona el tipo de conexión [1-3]" ;;
        "es:connection.prompt.scan_ble") echo "¿Deseas escanear dispositivos BLE cercanos?" ;;
        "es:installer.cancelled") echo "Instalación cancelada." ;;
        "es:installer.header.install_method") echo "Método de instalación" ;;
        "es:installer.header.installing_files") echo "Instalando archivos" ;;
        "es:installer.info.downloading") echo "Descargando desde GitHub (%s @ %s)..." ;;
        "es:installer.info.install_dir_selected") echo "Directorio de instalación: %s" ;;
        "es:installer.intro") echo "Este instalador te ayuda a configurar MeshCore Packet Capture." ;;
        "es:installer.lang.active") echo "Idioma configurado en %s." ;;
        "es:installer.lang.unsupported") echo "El idioma %s no está disponible. Se usará inglés." ;;
        "es:installer.menu.install_method") echo "Elige tu método de instalación preferido:\n\n  1) Servicio del sistema (recomendado para producción)\n     • Se inicia automáticamente al encender\n     • Administrado por systemd (Linux) o launchd (macOS)\n     • Reinicio automático ante fallas\n\n  2) Contenedor Docker (recomendado para desarrollo/pruebas)\n     • Entorno aislado\n     • Fácil de actualizar y administrar\n     • Funciona en Linux, macOS y Windows\n\n  3) Solo instalación manual\n     • Sin inicio automático\n     • Ejecución manual cuando sea necesario" ;;
        "es:installer.prompt.dir_exists") echo "El directorio ya existe. ¿Reinstalar o actualizar?" ;;
        "es:installer.prompt.install_dir") echo "Directorio de instalación" ;;
        "es:installer.prompt.install_method") echo "Elige el método de instalación [1-3]" ;;
        "es:installer.summary.config") echo "Archivo de configuración: %s/.env.local" ;;
        "es:installer.summary.header") echo "¡Instalación completa!" ;;
        "es:installer.summary.install_dir") echo "Directorio de instalación: %s" ;;
        "es:installer.title") echo "Instalador de MeshCore Packet Capture v%s" ;;
        *) return 1 ;;
    esac
}
# >>> I18N LOOKUP END
...
translate() {
    ...
    shift
    local text
    text="$(translate_lookup "$CURRENT_LANG" "$key")"
    if [ -z "$text" ] && [ "$CURRENT_LANG" != "en" ]; then
        text="$(translate_lookup "en" "$key")"
    fi
    if [ -z "$text" ]; then
        text="$key"
    fi
...
select_connection_type() {
    echo ""
    print_header "$(translate "connection.header")"
    echo ""
    say "connection.menu"
    echo ""
    ...
    while true; do
        local choice=$(prompt_input "$(translate "connection.prompt.choice")" "$DEFAULT_CHOICE")
        case $choice in
            1)
                CONNECTION_TYPE="ble"
                print_info "$(translate "connection.chosen.ble")"
                echo ""
                
                if prompt_yes_no "$(translate "connection.prompt.scan_ble")" "y"; then
                    while true; do
            ...
            2)
                ...
                print_info "$(translate "connection.chosen.serial")"
                echo ""
            ...
            3)
                CONNECTION_TYPE="tcp"
                print_info "$(translate "connection.chosen.tcp")"
                echo ""
...
main() {
    set_language
    print_header "$(translate "installer.title" "$SCRIPT_VERSION")"
    ...
    INSTALL_DIR=$(prompt_input "$(translate "installer.prompt.install_dir")" "$DEFAULT_INSTALL_DIR")
    INSTALL_DIR="${INSTALL_DIR/#\~/$HOME}"  # Expand tilde
    
    print_info "$(translate "installer.info.install_dir_selected" "$INSTALL_DIR")"
    
    # Check if directory exists
    UPDATING_EXISTING=false
    if [ -d "$INSTALL_DIR" ]; then
        if prompt_yes_no "$(translate "installer.prompt.dir_exists")" "n"; then
            print_info "Updating existing installation..."
            UPDATING_EXISTING=true
        else
            print_error "$(translate "installer.cancelled")"
            exit 1
        fi
    fi
    ...
    print_header "$(translate "installer.header.installing_files")"
    ...
    if [ -n "${LOCAL_INSTALL}" ]; then
        ...
        print_info "$(translate "installer.info.downloading" "$REPO" "$BRANCH")"
    ...
    print_header "$(translate "installer.header.install_method")"
    
    say "installer.menu.install_method"
    echo ""
    
    INSTALL_METHOD=$(prompt_input "$(translate "installer.prompt.install_method")" "1")
    ...
    # Final summary
    print_header "$(translate "installer.summary.header")"
    say "installer.summary.install_dir" "$INSTALL_DIR"
    echo ""
    say "installer.summary.config" "$INSTALL_DIR"
    echo ""
